# TriShield: Autonomous Aerial Drone Swarm System
**Comprehensive Study Guide & Architecture Documentation**
TriShield is a conceptually verified, containerized architecture that bridges pure mathematical theories (Particle Swarm Optimization & Genetic Algorithms) into an industry-grade distributed network (ROS 2 Humble). This project proves that a swarm of physically distinct aerial drones (UAVs) can dynamically allocate tasks and navigate 3D environments without a central command server.

---

## Phase 1: The Core AI Mathematics (`trishield_core`)

To ensure the logic is highly testable and framework-agnostic, the raw mathematics and flight physics of the swarm were written in pure Python.

### 1. Hardware Abstraction & Flight Bounds (`agent.py`)
We created the `UAVAgent` class wrapper to enforce kinematic constraints and altitude holds for 3D aerial drone movement ($X, Y, Z$).

### 2. Aerial Task Allocation (`ga_allocator.py`)
This script uses a **Genetic Algorithm** to decide which drone handles which threat (e.g., stopping a rogue drone vs. providing emergency aerial support).
- **The Logic:** It calculates 3D distance/ETA to the target and evaluates drone battery states to assign optimal targets.
...
- **Micro Avoidance (Repulsion):** During flight, the algorithm iterates over all peer drones. If a peer enters a 3.0-meter radius, an inverse vector is added to the drone's velocity, effortlessly diverting flight paths around each other to prevent mid-air collisions.

---

## Phase 2: Distributed ROS 2 Integration (`trishield_ros2`)

A Python simulation uses a single `for-loop` to process data sequentially. Physical drones cannot do this. You must deploy independent code to each drone's internal processor. We achieved this distributed setup using **ROS 2**.
...
This single script is deployed onto every physical drone in the fleet. It is completely independent and autonomous.
...
Rather than opening multiple terminal windows and running `ros2 run` for each robot, the `launch` descriptor spins up the Blackboard, `UAV_1`, `UAV_2`, `UAV_3`, `UAV_4`, and `UAV_5` completely simultaneously and threads their output into a unified terminal feed.
